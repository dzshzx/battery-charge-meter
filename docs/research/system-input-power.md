...
**2026-08-05 eva-02 实机验证更新**：目标机 REDMI Book Pro 14 2025（Core Ultra 5 225H）上，Intel **Psys**（RAPL Platform 域，`MSR_PLATFORM_ENERGY_STATUS`，MSR `0x64D`）经官方签名的 [PawnIO](https://pawnio.eu/) 内核驱动**已实测确认可读**（详见下文与 `2026-08-05-eva02-empirical-probe.md`）。在此前提下，`整机输入 ≈ Psys + 电池端充电功率` 是一条精度可用的**估算**路径，未测项只剩充电路径损耗（典型 5–10%）。但必须明确：这是两个实测量的**组合估算**，不是入口电表的直接实测值，仍不能标成 `Measured System Input Power`。
...
| **估算整机输入功率** | Estimated system input power | `Psys + max(电池端充电功率, 0)`。**测量口径：Estimated**（两个 Measured 量的组合，不是入口电表直接实测）。未测项只剩充电路径损耗（典型 5–10%）。目标机 2026-08-05 实测：空闲 37.14 W、14 核满载 60.14 W。 |
...
1. **默认仍保持一个直接运行的 EXE。** 内置 `BatteryTerminalProvider`，把当前数值更名为“电池端充电功率”；自动探测 PMI/EMI，但探测不到就保持 `Unsupported`。同一批免驱能力可再加一个 EMI `RAPL_Package0_PKG` provider，标注“CPU 包功率”，标准用户权限即可读，无需驱动无需管理员（2026-08-05 eva-02 实测确认）。
...
**2026-08-05 补充**：以上判断针对**实测**入口瓦数依然成立——目标机没有任何标准接口能给出 `Measured System Input Power`。但已找到一条精度可用的**估算**替代路径：`Psys + max(电池端充电功率, 0)`，两个分量均为 Measured，组合结果按 `Estimated` 标注即可对外提供，不必因缺少入口电表而只报 `Unsupported`。

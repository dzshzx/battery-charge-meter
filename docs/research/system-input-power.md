...
> 历史研究记录（2026-08-05 实机验证版）：本文保存选型时的数据源调研与测量推理。术语与公式以 `CONTEXT.md`（唯一术语表）和 README「功率口径」为准；本文「推荐的产品数据模型」一节是当时的建议，已被 CONTEXT.md 取代——对照：充电口实测输入功率 → 整机输入功率（System Input Power）、平台功耗（不含电池充电）→ 平台功率（Platform Power）、市电侧输入功率 → 墙端输入功率（Wall Input Power）；平衡式以 README「功率口径」的写法为准。
...
**实机验证与精度边界（2026-09-22 修订）**：REDMI Book Pro 14 2025（Core Ultra 5 225H）上，Intel **Psys**（RAPL Platform 域，`MSR_PLATFORM_ENERGY_STATUS`，MSR `0x64D`）经官方签名的 [PawnIO](https://pawnio.eu/) 内核驱动已确认可读。历史 0.02–0.04 W 的交叉校验比较的是 MSR 与 EMI 的 **CPU 包功率**，用于检查 CPU 包的单位换算，不能证明 Psys 的整机覆盖范围和绝对精度。`整机输入 ≈ Psys + 电池端净功率（带符号）` 保留为组合估算，仍受传感器误差、采样时间差、平台覆盖和未测转换损耗影响。

2026-09-20 纯电池供电的 127.42 秒观察中，平台均值为 17.66 W，电池放电均值为 17.08 W，容量变化折算为 17.15 W。两源平均值相近；期间电池数值曾约 28 秒没有变化，不能将每秒读取当作每秒独立测量。2026-09-22 插电诊断在提权后读到约 60–67 W 的整机估算，与用户电流表报告的约 60 W 属同一量级，但并非同步记录，不能据此宣称误差上限。下文保留历史研究过程，涉及精度的结论以本段和 `CONTEXT.md` 的测量边界为准。
...
- 上游 Linux 的 [REDMI WMI 驱动](https://github.com/torvalds/linux/blob/master/drivers/platform/x86/redmi-wmi.c)目前只处理 OEM 功能/事件；没有公开整机瓦数读取。**2026-08-05 测试机 实机 + 一手来源调研双重证实**：小米 EC/WMI（Bitland MIFS）确实没有把充电芯片的输入电流、电压或功率暴露给 Windows——不再是"尚不能证明"，而是**证伪**（`MIFS` 未注册任何 WMI GUID，`EcIoSdk.dll` 仅有日志接口，两个独立逆向项目全命令表扫描均未发现功率寄存器）。
- 该机型最值得实机验证的顺序：标准 `ACPI000D`/PMI → EMI 的入口总 rail → `Psys` (`0x64D`) → PMT 元数据/通道，**已于 2026-08-05 在 测试机 上完整执行**。结果：`ACPI000D`/PMI 均证伪（不存在或为存根）；EMI 存在但只有 4 条 CPU 内部 RAPL 通道，无入口总 rail；`Psys`（`0x64D`）**实测确认可读**，经 PawnIO 驱动给出真实平台功率；PMT 驱动虽在运行，但暴露的是未公开的私有接口 GUID，无公开用户态 API，判定不可作为可发布产品依赖。

## 开源与现成方案横向比较

广泛检索后的关键发现不是一个“无需权限即可读取所有 Windows 笔记本充电口功率”的现成库，而是几条边界不同、可组合的数据源。尤其是 Intel `Psys`：它在目标 Arrow Lake-H 平台上并非理论猜测，Intel PCM、WinPowerMonitor 和 LibreHardwareMonitor 都已有公开实现，且**已于 2026-08-05 在目标机 测试机 上实机验证可读**（见上文）；真正的门槛是 Windows 的 MSR 访问必须经过内核驱动——PCM、WinPowerMonitor 各自的驱动仍受 test-signing 限制，但 LibreHardwareMonitor 现行的 PawnIO 路线不受此限制（官方驱动已数字签名）。
...
## 推荐的产品数据模型（历史建议，术语已由 CONTEXT.md 取代）
...
1. **便携 EXE 与安装包均已发布。** 两种形式使用同一个应用程序集；`BatterySensor` 把电池读数归一为带符号的“电池端净功率”，`EmiSensor` 在标准用户权限下读取 EMI `RAPL_Package0_PKG` 并显示“CPU 包功率”。
2. **Psys 已作为可选能力发布。** `PawnIoSensor` 通过用户另行安装的官方 PawnIO 驱动读取平台计数器，并在 EMI 可用时用 CPU 包计数器交叉校验单位。`PowerSources` 在外部供电时按“平台功率 + 电池端净功率（带符号）”生成“估算整机输入功率”；未安装驱动、未提权、计数器不推进或校验失败时会保持不可用并给出原因。应用本身不安装驱动。
3. **直接入口电表与 PD 合同 provider 尚未实现。** PMI/OEM 入口电表只有在测量边界可证明时才能新增；PD 合同只能作为连接信息，软件模型必须标为 `Estimated` 并附带校准时间、数据源和置信度。

**产品数据源范围：使用机内软件路径；2026-09-22 起可用用户已有电流表进行验收对照，尚未集成外置硬件 provider。历史未采用的 provider：**
...
**当前实现**：以上判断针对**实测**入口瓦数依然成立——目标机没有任何标准接口能给出 `Measured System Input Power`。从 v1.2.1 起，应用已提供精度可用的**估算**替代路径：`Psys + 电池端净功率（带符号）`，两个分量均为 Measured；电池补充适配器时净功率为负，必须从 Psys 中扣除。组合结果始终按 `Estimated` 标注，不会因缺少入口电表而冒充实测值。

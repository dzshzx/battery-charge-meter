...
| Protected logon startup | Follow `protected-autostart.md`: enable startup, replace the user-folder EXE with the ordinary token, run the logon task, upgrade and uninstall. The task only runs the admin-only copy, whose hash tracks the installed version. | 2026-09-26, NERV, candidate `582b540`: passed; see `protected-autostart.md`. Sign-out/sign-in not performed. |

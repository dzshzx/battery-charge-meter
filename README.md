...
1. [PawnIO](https://pawnio.eu/) 驱动。它是免费、开源、经数字签名的通用内核
   驱动，请自行从官网下载安装包安装；本程序不会代为安装驱动。
2. 与 EXE 放在同一目录的 `IntelMSR.bin`（PawnIO 官方模块，随 Release 附带）。

缺少任一项时，这两个指标显示不可用并说明缺什么，其余功能不受影响。
